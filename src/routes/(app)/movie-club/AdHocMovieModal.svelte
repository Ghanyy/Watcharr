<script lang="ts">
	...
	let isSelectedMovieExcluded = false;
	let exclusionMessage = "";
	let checkingExclusion = false;

	// Form fields
	let name = "Ad-hoc Movie Session";
	let description = "";
	let adHocDurationHours = 24;
	...
	onMount(() => {
		// Focus search input
		const searchInput = document.getElementById("adhoc-search-input");
		if (searchInput) {
			searchInput.focus();
		}
	...
	async function createAdHocCycle() {
		if (submitting || !selectedMovie || !name.trim()) return;

		if (isSelectedMovieExcluded) {
			notify({ text: "Cannot select a previous winner", type: "error" });
			return;
		}

		submitting = true;
		const nid = notify({ text: "Creating ad-hoc session...", type: "loading" });

		try {
			const response = await axios.post("/movie-club/adhoc-cycle", {
				name: name.trim(),
				description: description.trim(),
				contentId: selectedMovie.tmdbId,
				adHocDurationHours: adHocDurationHours,
			});

			notify({
				id: nid,
				text: "Ad-hoc movie session created!",
				type: "success",
			});
			...
			console.error("Failed to create ad-hoc cycle:", err);
			const message =
				err.response?.data?.error || "Failed to create ad-hoc session";
	...
	function handleKeydown(event: KeyboardEvent) {
		if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) {
			if (selectedMovie && !isSelectedMovieExcluded) {
				createAdHocCycle();
			}
		}
	}
	...
	$: canSubmit =
		selectedMovie && name.trim() && !isSelectedMovieExcluded && !submitting;
</script>
	...
	{:else}
		...
		<div class="session-section">
			...
			<div class="session-details">
				<div class="form-group">
					...
					<input
						...
						type="text"
						bind:value={name}
						placeholder="Ad-hoc Movie Session"
						maxlength="100"
						disabled={submitting || isSelectedMovieExcluded}
				...
				<div class="form-group">
					...
					<textarea
						...
						placeholder="Describe this ad-hoc session..."
						maxlength="500"
						disabled={submitting || isSelectedMovieExcluded}
						on:keydown={handleKeydown}
					></textarea>
				...
				<div class="form-group">
					<label for="session-duration">Watching Duration (Hours)</label>
					<input
						id="session-duration"
						type="number"
						bind:value={adHocDurationHours}
						min="1"
						max="168"
						disabled={submitting || isSelectedMovieExcluded}
					/>
					<small
						>How long should this session remain active? (1-168 hours)</small
					>
				</div>
			</div>
			...
			<div class="action-buttons">
				...
				<button
					class="submit-btn"
					on:click={createAdHocCycle}
					disabled={!canSubmit}
				>
					<Icon icon="sparkles" />
					{#if submitting}
						Creating...
					{:else if checkingExclusion}
						Checking...
					{:else if isSelectedMovieExcluded}
						Cannot Create
					{:else}
