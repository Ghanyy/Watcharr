<script lang="ts">
	...
	import { UserType, type Image, type Profile, type MatrixAccountInfo } from "@/types";
	...
	function getAccountTypeInfo(accountType?: string, isAutoGenerated?: boolean, asManagedUser?: boolean) {
		if (accountType === "appservice") {
			return {
				...
				icon: "🤖",
				color: "#4A90E2",
				features: ["Movie Club Chats", "No External Access", "Automatic Management"]
			};
		} else if (accountType === "personal") {
			return {
				type: isAutoGenerated ? "Personal (Auto-Generated)" : "Personal (Custom Linked)",
			...
			return {
				type: "Unknown",
				description: isAutoGenerated ? "Auto-generated account" : "Custom linked account",
				icon: "❓",
				color: "#6B7280",
			...
			{#if config?.MOVIE_CLUB?.matrix?.enabled}
				...
				{:else if matrixInfo}
					{#if matrixInfo.hasMatrixAccount}
						{@const accountTypeInfo = getAccountTypeInfo(matrixInfo.accountType, matrixInfo.isAutoGenerated, matrixInfo.asManagedUser)}
						<Setting
						...
						>
							<div style="display: flex; flex-direction: column; gap: 15px;">
								<!-- Account Type Badge -->
								<div class="matrix-account-type" style="border-left: 3px solid {accountTypeInfo.color};">
									<div class="account-type-header">
										<span class="account-type-icon">{accountTypeInfo.icon}</span>
										<div class="account-type-info">
											<span class="account-type-name">{accountTypeInfo.type}</span>
											<span class="account-type-desc">{accountTypeInfo.description}</span>
										</div>
									</div>
								...
								<div style="display: flex; flex-direction: column; gap: 5px;">
									<span style="font-weight: 500; font-family: monospace; font-size: 14px;"
										>{matrixInfo.matrixUserId}</span
									>
									{#if matrixInfo.accountType === "appservice"}
										<span style="font-size: 12px; opacity: 0.7; color: {accountTypeInfo.color};">
									...
									{:else if matrixInfo.accountType === "legacy"}
										<span style="font-size: 12px; opacity: 0.7; color: {accountTypeInfo.color};">
											⚠️ Legacy account format - consider recreating for latest features
										</span>
									{:else}
								...
								<div style="display: flex; gap: 10px; flex-wrap: wrap;">
									...
									>
										{#if matrixUnlinkLoading}
											<Spinner />
										{:else}
											{matrixInfo.accountType === "appservice" ? "Remove Account" : "Unlink Account"}
										{/if}
									</button>
								</div>
							...
							<div style="display: flex; flex-direction: column; gap: 20px;">
								...
								<div class="matrix-account-options">
									<div class="account-option recommended">
										<div class="option-header">
											...
											<div class="option-info">
												<span class="option-name">Application Service Account</span>
												<span class="option-badge">Recommended</span>
											</div>
										</div>
										<p class="option-description">
											Virtual Matrix user managed automatically by Watcharr. Perfect for users who only need Movie Club features.
										</p>
									...
									<div class="account-option">
										...
										<p class="option-description">
											Real Matrix account that you can access with Element Web and other Matrix clients.
										</p>
										<div class="option-features">
										...
										<div class="personal-account-options">
											<div class="sub-option">
												...
												<p>Let Watcharr create a real Matrix account with credentials you can export</p>
												<p class="note">Note: Requires shared secret configuration on Matrix server</p>
											</div>
											<div class="sub-option">
