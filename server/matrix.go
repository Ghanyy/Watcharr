...
func LinkCustomMatrixUser(db *gorm.DB, watcharrUserID uint, matrixUserID string, accessToken string) error {
	...
	// Check for existing Matrix users for this Watcharr user (both V1 and V2)
	var existingLegacyUsers []MatrixUser
	var existingV2Users []MatrixUserV2
	var totalExistingUsers int
	
	if err := db.Where("user_id = ?", watcharrUserID).Find(&existingLegacyUsers).Error; err != nil {
		slog.Warn("Failed to check existing legacy Matrix users", 
			"watcharr_user_id", watcharrUserID, 
			"error", err)
	} else {
		totalExistingUsers += len(existingLegacyUsers)
		for i, user := range existingLegacyUsers {
			slog.Info("Existing legacy Matrix user", 
				"index", i,
	...
	if err := db.Where("user_id = ?", watcharrUserID).Find(&existingV2Users).Error; err != nil {
		slog.Warn("Failed to check existing V2 Matrix users", 
			"watcharr_user_id", watcharrUserID, 
			"error", err)
	} else {
		totalExistingUsers += len(existingV2Users)
		for i, user := range existingV2Users {
			slog.Info("Existing V2 Matrix user", 
				"index", i,
				"matrix_user_id", user.MatrixUserID,
				"is_auto_generated", user.IsAutoGenerated,
	...
	slog.Info("Found existing Matrix users before linking", 
		"watcharr_user_id", watcharrUserID, 
		"legacy_count", len(existingLegacyUsers),
		"v2_count", len(existingV2Users),
		"total_count", totalExistingUsers)
	...
	// Store custom user mapping using V2 model
...
// RemoveAutoGeneratedUser removes an auto-generated Matrix user (supports both V1 and V2 models)
func RemoveAutoGeneratedUser(db *gorm.DB, watcharrUserID uint) error {
	errContext := map[string]interface{}{
		"watcharr_user_id": watcharrUserID,
	}

	// First check V2 model
	var matrixUserV2 MatrixUserV2
	if err := db.Where("user_id = ? AND is_auto_generated = true", watcharrUserID).First(&matrixUserV2).Error; err == nil {
		// Found V2 auto-generated user
		errContext["matrix_user_id"] = matrixUserV2.MatrixUserID
		errContext["account_type"] = matrixUserV2.AccountType

		slog.Info("Removing auto-generated Matrix user V2 from all rooms",
			"watcharr_user_id", watcharrUserID,
			"matrix_user_id", matrixUserV2.MatrixUserID,
			"account_type", matrixUserV2.AccountType)
		
		if err := RemoveUserFromMatrixRooms(db, matrixUserV2.MatrixUserID, watcharrUserID); err != nil {
			slog.Warn("Failed to remove V2 user from Matrix rooms", 
				"watcharr_user_id", watcharrUserID, 
				"matrix_user_id", matrixUserV2.MatrixUserID,
				"error", err)
		}

		// Handle AS vs personal account deactivation
		if matrixUserV2.AccountType == "appservice" || matrixUserV2.ASManagedUser {
			// AS account - delete virtual user via AS
			if appServiceManager != nil {
				if err := appServiceManager.DeleteUser(watcharrUserID); err != nil {
					slog.Warn("Failed to delete AS virtual user", 
						"matrix_user_id", matrixUserV2.MatrixUserID, 
						"error", err)
				}
			}
		} else if matrixUserV2.AccountType == "personal" && matrixUserV2.IsAutoGenerated {
			// Auto-generated personal account - deactivate on Matrix server
			if err := DeactivateMatrixUser(db, matrixUserV2.MatrixUserID); err != nil {
				slog.Warn("Failed to deactivate auto-generated Matrix user", 
					"matrix_user_id", matrixUserV2.MatrixUserID, 
					"error", err)
			}
		}
		...
		if err := db.Delete(&matrixUserV2).Error; err != nil {
			return logAndReturnError("database_delete_v2", err, errContext)
		}

		slog.Info("Removed auto-generated Matrix user V2",
			"watcharr_user_id", watcharrUserID,
			"matrix_user_id", matrixUserV2.MatrixUserID)
	...
	// Check legacy V1 model if V2 not found
	var matrixUser MatrixUser
	...
	// Attempt to deactivate user on Matrix server (legacy auto-generated account)
	slog.Info("Attempting to deactivate legacy auto-generated Matrix user on server",
		"watcharr_user_id", watcharrUserID,
		"matrix_user_id", matrixUser.MatrixUserID)
