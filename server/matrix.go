...
func CreateMatrixUser(db *gorm.DB, watcharrUserID uint, username string) (*MatrixUser, error) {
	...
	if Config.MOVIE_CLUB.Matrix.RegistrationSecret != "" {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "User ID already taken") || 
			   ...
			   strings.Contains(err.Error(), "M_USER_IN_USE") {
				...
				slog.Info("Matrix user already exists, attempting reactivation", 
					"watcharr_user_id", watcharrUserID, 
					"matrix_user_id", matrixUserID)
				
				// Attempt to reactivate the existing user
				registrationResponse, err = ReactivateMatrixUser(matrixUserID, password)
				if err != nil {
					return nil, logAndReturnError("matrix_reactivation", fmt.Errorf(
						"failed to create new user (already exists) and failed to reactivate existing user: %w", err), errContext)
				}
				
				wasReactivated = true
				slog.Info("Successfully reactivated existing Matrix user", 
					"watcharr_user_id", watcharrUserID, 
		...
		if wasReactivated {
			slog.Info("Reactivated existing Matrix user and stored credentials", 
				"watcharr_user_id", watcharrUserID, 
				"matrix_user_id", registrationResponse.UserID)
		} else {
...
func ReactivateMatrixUser(matrixUserID, newPassword string) (*SharedSecretRegistrationResponse, error) {
	...
	// Make HTTP request to Matrix server login endpoint
	url := fmt.Sprintf("%s/_matrix/client/r0/login", getMatrixServerURL())
	resp, err := http.Post(url, "application/json", bytes.NewBuffer(requestJSON))
	if err != nil {
		return nil, logAndReturnError("http_request", err, errContext)
	}
	...
	// Check status code
	if resp.StatusCode != http.StatusOK {
		return nil, logAndReturnError("matrix_login_failed", 
			fmt.Errorf("login failed with status %d: %s", resp.StatusCode, responseBody.String()), errContext)
	}
	...
	if err := json.Unmarshal(responseBody.Bytes(), &loginResponse); err != nil {
		return nil, logAndReturnError("json_unmarshal", err, errContext)
	}
	
	// Validate response
	if loginResponse.UserID == "" || loginResponse.AccessToken == "" {
		return nil, logAndReturnError("invalid_login_response", 
			errors.New("missing required fields in login response"), errContext)
	}
	
	// Convert to registration response format for compatibility
	reactivationResponse := SharedSecretRegistrationResponse{
