...
func CreateMatrixUser(db *gorm.DB, watcharrUserID uint, username string) (*MatrixUser, error) {
	...
	// Create user on Matrix server using admin API
	req := &mautrix.ReqCreateUser{
		Username: strings.TrimPrefix(matrixUserID, "@"),
		Password: password,
	}

	resp, err := matrixClient.CreateUser(context.Background(), req)
	if err != nil {
		return nil, fmt.Errorf("failed to create Matrix user: %w", err)
	}

	// Login to get access token
	loginResp, err := matrixClient.Login(context.Background(), &mautrix.ReqLogin{
