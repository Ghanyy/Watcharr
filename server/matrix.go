...
func CreateMatrixUser(db *gorm.DB, watcharrUserID uint, username string) (*MatrixUser, error) {
	...
	if Config.MOVIE_CLUB.Matrix.RegistrationSecret != "" {
		...
		if existingMatrixUser.ID != 0 && existingMatrixUser.DeletedAt.Valid {
			...
			matrixUser.MatrixUserID = registrationResponse.UserID
...
func UnlinkMatrixUser(db *gorm.DB, watcharrUserID uint) error {
	...
	// Remove the Matrix user record from Watcharr database
	if matrixUser.IsAutoGenerated {
		// For auto-generated accounts, use soft delete so they can be reactivated later
		if err := db.Delete(&matrixUser).Error; err != nil {
			return logAndReturnError("database_soft_delete", err, errContext)
		}
		slog.Info("Soft deleted auto-generated Matrix user record (can be reactivated)",
			"watcharr_user_id", watcharrUserID,
			"matrix_user_id", matrixUser.MatrixUserID)
	} else {
		// For custom accounts, hard delete since they won't be recreated by Watcharr
		if err := db.Unscoped().Delete(&matrixUser).Error; err != nil {
			return logAndReturnError("database_hard_delete", err, errContext)
		}
		slog.Info("Hard deleted custom Matrix user record",
			"watcharr_user_id", watcharrUserID,
			"matrix_user_id", matrixUser.MatrixUserID)
	}
