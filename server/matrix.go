...
func LinkCustomMatrixUser(db *gorm.DB, watcharrUserID uint, matrixUserID string, accessToken string) error {
	...
	// Remove any existing Matrix user for this Watcharr user
	if err := db.Where("user_id = ?", watcharrUserID).Delete(&MatrixUser{}).Error; err != nil {
		slog.Warn("Failed to remove existing Matrix user", 
			"watcharr_user_id", watcharrUserID, 
			"error", err)
	}
