...
async function testMovieClub() {
	...
	try {
		...
		const cycleResponse = await axios.get(`${baseURL}/movie-club/current`, {
			headers,
		});
		console.log(
			"✅ Current cycle:",
			cycleResponse.data.cycle?.phase || "No active cycle",
		);

		// Test nomination (if in nomination phase)
		if (
			cycleResponse.data.cycle?.phase === "nomination" &&
			cycleResponse.data.canNominate
		) {
			console.log("Testing nomination...");
			const nominateResponse = await axios.post(
				`${baseURL}/movie-club/nominate`,
				{
		...
		if (
			cycleResponse.data.cycle?.phase === "voting" &&
			cycleResponse.data.canVote
		) {
			console.log("Testing voting...");
			const voteResponse = await axios.post(
				`${baseURL}/movie-club/vote`,
				{
					votes: [
		...
		if (cycleResponse.data.cycle?.phase === "watching") {
			...
			const resultsResponse = await axios.get(`${baseURL}/movie-club/results`, {
				headers,
			});
			console.log(
				"✅ Results retrieved:",
